...
Actions2Wheels::Actions2Wheels()
{
    MotorFactory motorFactory;
    m_motor = motorFactory.Create(MOTOR_TYPE);
    std::cout << "Actions object 2 Wheels created" << std::endl;
}

void Actions2Wheels::Stop()
{
    /*sets velocity to 0 and releases the motor*/
...
void Actions2Wheels::DriveForward(const int velocity)
{
...
void Actions2Wheels::DriveBackward(const int velocity)
{
...
void Actions2Wheels::TurnLeft(const int velocity)
{
...
void Actions2Wheels::TurnRight(const int velocity)
{
...
bool Actions2Wheels::Turn90DegreesRight(const float gamma, const int velocity)
{
...
bool Actions2Wheels::Turn90DegreesLeft(const float gamma, const int velocity)
{
    /*When a target object is scanned by the camera, the vehicle turns by
...
bool Actions2Wheels::CorrectForDistance(const float distToWall, const int velocity, float threshold)
{
...
bool Actions2Wheels::DriveToWall(const float distToWall, const int velocity, float threshold)
{
...
bool Actions2Wheels::CorrectForAngleBetweenWorldAndRobotFrame(const float gamma, const int sign, const int velocity, float thresholdAngle)
{
...
std::unique_ptr<ActionsInterface> Actions2Wheels::Instantiate()
{
    /*Sets pointer for MotorFactory*/
    return std::make_unique<Actions2Wheels>();
}