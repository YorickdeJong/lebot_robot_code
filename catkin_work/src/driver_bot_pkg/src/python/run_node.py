...
import actions.navigationData as navigationData
import numpy as np
import rospy
...
class RunScript:
    def __init__(self):
        ...
        self.action = navigationData.Actions()
    ...
    def AdjustPos(self):
        ...
        while self.distToWall < 0.4:
            ...
            if (not self.rightSide and self.sign == -1 and (self.gamma > error or self.minDistance < minDist)):  # 1
                ...
                while self.action.checkGamma:
                    self.action.correctionAngle(self.gamma, 1)
                self.action.checkGamma = True
            ...
            if self.distToWall < minDistToWall:
                ...
                while self.action.checkDistToWall:
                    self.action.correctionDistance(self.distToWall)
                self.action.checkDistToWall = True

                while self.action.checkGamma:    
                    self.action.correctionAngle(self.gamma, sign)
                self.action.checkGamma = True

        # check for target object
        while self.action.checkGamma:
            self.action.turning90Degrees(self.gamma, self.checkOverlap)

        # if distance to object infront of the robot is larger than 40 cm, move forward
        self.action.driveForward(0.8)
